...
export function BuyRequestFilters({ sets }: { sets: SetOption[] }) {
  ...
  function update(patch: Record<string, string | undefined>) {
    ...
    next.delete("page");
    startTransition(() => router.push(`/buy-requests?${next.toString()}`));
  }

  function reset() {
    startTransition(() => router.push("/buy-requests"));
  }
  ...
  function onPriceSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    update({ minPrice: ((fd.get("minPrice") as string) ?? "").trim() || undefined });
  }
